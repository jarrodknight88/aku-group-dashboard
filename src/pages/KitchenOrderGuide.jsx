...
import { useIsMobile, MStatGrid, MList, MPill, MLocSelect } from '../components/mobile.jsx'
import { fmtRange } from '../lib/dates.js'

/* Kitchen Order Guide (POC — handoff spec Jul 6 2026). Pick a delivery
   date, generate, adjust pack counts, confirm. Sunday targets render as the
   Restaurant Depot run list (no US Foods delivery on Sunday — shop
   Saturday). Everything estimate-based wears a badge until the chef
   verifies portions and pack sizes. */

const ET = 'America/New_York'

/** Today's date string in ET (business timezone). */
function todayET() {
...
function cutoffEpoch(dateStr) {
  ...
  const offsetHours = (12 - etHour + 24) % 24 // hours ET lags UTC (4 in summer, 5 in winter)
...
export default function KitchenOrderGuide() {
  ...
  const [target, setTarget] = useState(() => addDaysStr(todayET(), 1)) // default: tomorrow
  const [guide, setGuide] = useState(null) // null = none yet for this date
  ...
  const isDepot = dowOf(target) === 0 // Sunday: no US Foods delivery — Depot run
  ...
  /** Depot display: buy quantities in retail units, not US Foods cases. */
  const depotQty = (l) => {
  ...
  const copyText = async () => {
    if (!guide) return
    const title = isDepot
      ? `Restaurant Depot run — ${fmtRange(target, target)} (shop Saturday)`
      : `US Foods order — deliver ${fmtRange(target, target)} (order by 7pm ${fmtRange(orderDay, orderDay)})`
    const lines = guide.lines
      .filter((l) => l.adjusted_packs > 0 || l.forecast_need != null)
      .map((l) => (isDepot ? `• ${l.ingredients.name}: ${depotQty(l)}` : `• ${l.ingredients.name}: ${l.adjusted_packs} × ${l.ingredients.pack_label}`))
    try {
    ...
    <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 8, alignItems: isMobile ? 'stretch' : 'center' }}>
      ...
      {activeLocs.length > 1 && (
        <MLocSelect
          value={locId ?? ''}
          onChange={setLocId}
          options={activeLocs.map((l) => ({ value: l.id, label: l.name }))}
        />
      )}
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: colors.pageBg, color: colors.ink }}>
      <AppHeader active="kitchen" />
      <div style={{ maxWidth: layout.maxWidth, margin: '0 auto', padding: '20px 26px 48px' }}>
        <Crumbs items={[{ label: 'Company', to: '/' }, { label: 'Kitchen' }, { label: 'Order Guide' }]} />
        <PageTitle
          title="Kitchen Order Guide"
          meta={<>Forecast-driven ordering · demand from Toast day-of-week averages · POC — portions are chef-verifiable estimates</>}
          right={controls}
        />
        ...
        {isDepot ? (
          <div style={{ padding: '11px 15px', background: '#FBF3DC', border: '1px solid #EAD9A8', borderRadius: 11, color: '#8A6D1A', fontSize: 12.5, fontWeight: 600, marginBottom: 14 }}>
            🛒 No US Foods delivery on Sundays — this is the <b>Restaurant Depot run list</b>. Shop Saturday; quantities shown in retail units.
          </div>
        ) : (
          <div style={{ padding: '11px 15px', background: '#E8EEF6', border: `1px solid ${colors.brandTint4}`, borderRadius: 11, color: colors.brand, fontSize: 12.5, fontWeight: 600, marginBottom: 14 }}>
            Delivery {fmtRange(target, target)} — order by <b>7:00 PM ET {fmtRange(orderDay, orderDay)}</b>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 16 }}>
          ...
          {guide && (
            <div onClick={copyText} style={btn(false, false)}>
              {copied ? '✓ Copied' : isDepot ? 'Copy run list' : 'Copy as text'}
            </div>
          )}
          {guide && (
        ...
        {guide && (
          <MStatGrid
            style={{ marginBottom: 14, maxWidth: isMobile ? undefined : 560 }}
            items={[
              { label: isDepot ? 'Run list items' : 'Order lines', value: guide.lines.length, sub: <span>buffer {Math.round(Number(guide.buffer_pct) * 100)}% while portions are estimates</span> },
              ...
              <div key={l.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderTop: i === 0 ? 'none' : `1px solid ${colors.pageBg}` }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 700 }}>
                    {l.ingredients.name}
                    {l.is_estimate && <span style={{ marginLeft: 6, fontSize: 9.5, fontWeight: 700, color: '#8A6D1A', background: '#FBF3DC', padding: '2px 6px', borderRadius: 5, verticalAlign: 'middle' }}>EST</span>}
                  </div>
                  ...
                  {guide.lines.map((l) => (
                    <tr key={l.id} style={{ borderTop: `1px solid ${colors.pageBg}` }}>
                      <td style={{ padding: '11px 18px', fontWeight: 700 }}>
                        {l.ingredients.name}
                        {l.is_estimate && <span style={{ marginLeft: 7, fontSize: 9.5, fontWeight: 700, color: '#8A6D1A', background: '#FBF3DC', padding: '2px 6px', borderRadius: 5 }}>EST</span>}
        ...
        <div style={{ fontSize: 11, color: colors.muted3, marginTop: 14, lineHeight: 1.55 }}>
          Forecast = day-of-week averages (June Toast data; switches to a rolling 6-week average automatically as nightly
          imports accumulate). Portions marked <b>EST</b> are Claude-estimated pending chef verification; pack sizes are
          assumed until checked against the US Foods order guide. Adjust any quantity before confirming — the gap between
          suggested and adjusted is what tunes the model. Shadow mode: keep ordering as usual for the first week or two
          and compare.
        </div>
      </div>
    </div>
